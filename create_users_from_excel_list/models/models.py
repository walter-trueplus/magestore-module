...
class create_users_from_excel_list(models.TransientModel):
    ...
    def parse_preview(self, options, count=1000):
        ...
        if self.res_model == u'res.users':
            ...
            except Exception, error:  # Due to lazy generators, UnicodeDecodeError (for
                # instance) may only be raised when serializing the
                # preview to a list in the return.
                _logger.debug("Error during parsing preview", exc_info=True)
                preview = None
    ...
    def do(self, fields, options, dryrun=False):
        ...
        if self.res_model == u'res.users':
            lst_err = []
            length_head = len(self.parse_preview(options, 1000)['headers'])
            length_pre = len(self.parse_preview(options, 1000)['preview'])
            preview_obj = self.parse_preview(options, 1000)['preview']
            headers_obj = self.parse_preview(options, 1000)['headers']
            try:
                data, import_fields = self._convert_import_data(fields, options)
                # Parse date and float field
                data = self._parse_import_data(data, import_fields, options)
                for v in range(0, length_head):
                    if headers_obj[v].lower() == u'name' or headers_obj[v].lower() == u'username':
                        for u in range(0, length_pre):
                            if preview_obj[u][v] == u'':
                                raise ValueError('The value in the column Name is not null.')
                    elif headers_obj[v].lower() == u'login':
                        for u in range(0, length_pre):
                            if preview_obj[u][v] == u'':
                                raise ValueError('The value in the column Login is not null.')
                    ...
                    for j in range(0, length_head):
                        if headers_obj[j].lower() == u'sale' or headers_obj[j].lower() == u'sales':
                            for i in range(0, length_pre):
                                if preview_obj[i][j] == u'0' or preview_obj[i][j] == u'1' or preview_obj[i][j] == u'2' \
                                        or preview_obj[i][j] == u'3' or preview_obj[i][j] == u'':
                                    if preview_obj[i][j] == u'0' or preview_obj[i][j] == u'':
                                        user_id = import_result['ids'][i]

                                        if user_id is not False:
                                            user_obj = self.env['res.users'].search([('id', '=', user_id)])
                                            if user_obj.id is not False:
                                                group_e = self.env.ref('sales_team.group_sale_manager',
                                                                       False)  # Manager
                                    ...
                                    if user_id is not False:
                                        ...
                                        if user_obj.id is not False:
                                            user_obj.unlink()
                                            lst_err.append(
                                                "The value of Sales column must be '0,1,2,3 or null' in row %s." % (
                                                    i + 1))
                                ...
                                if preview_obj[i][j] == u'0' or preview_obj[i][j] == u'1' or preview_obj[i][j] == u'2' \
                                    ...
                                    if preview_obj[i][j] == u'0' or preview_obj[i][j] == u'':
                                        ...
                                        if user_id is not False:
                                            user_obj = self.env['res.users'].search([('id', '=', user_id)])
                                            if user_obj.id is not False:
                                                group_e = self.env.ref('project.group_project_manager',
                                                                       False)  # Manager
                                                group_e.write({'users': [(3, user_obj.id)]})
                                    ...
                                    if user_id is not False:
                                        ...
                                        if user_obj.id is not False:
                                            user_obj.unlink()
                                            lst_err.append(
                                                "The value of Project column must be '0,1,2,3 or null' in row %s." % (
                                                    i + 1))
                                ...
                                if preview_obj[i][j] == u'0' or preview_obj[i][j] == u'1' or preview_obj[i][j] == u'2' \
                                    ...
                                    if preview_obj[i][j] == u'0' or preview_obj[i][j] == u'':
                                        ...
                                        if user_id is not False:
                                            user_obj = self.env['res.users'].search([('id', '=', user_id)])
                                            if user_obj.id is not False:
                                                group_e = self.env.ref('account.group_account_manager',
                                                                       False)  # Manager
                                                group_e.write({'users': [(3, user_obj.id)]})
                                                group_env = self.env.ref('account.group_account_user',
                                                                         False)  # Accountant
                                                group_env.write({'users': [(3, user_obj.id)]})
                                                group_env_1 = self.env.ref('account.group_account_invoice',
                                                                           False)  # Billing
                                                group_env_1.write({'users': [(3, user_obj.id)]})
                                    ...
                                    if user_id is not False:
                                        ...
                                        if user_obj.id is not False:
                                            user_obj.unlink()
                                            lst_err.append("The value of Account & Finance column "
                                                           "must be '0,1,2,3 or null' in row %s." % (
                                                               i + 1))
                            ...
                            for i in range(0, length_pre):
                                if preview_obj[i][j] == u'0' or preview_obj[i][j] == u'1' or preview_obj[i][j] == u'2' \
                                    ...
                                    elif preview_obj[i][j] == u'0' or preview_obj[i][j] == u'':
                                        user_id = import_result['ids'][i]
                                    ...
                                    if user_id is not False:
                                        ...
                                        if user_obj.id is not False:
                                            user_obj.unlink()
                                            lst_err.append(
                                                "The value of Employee column must be '0,1,2,3 or null' in row %s." % (
                                                    i + 1))
                                ...
                                if preview_obj[i][j] == u'0' or preview_obj[i][j] == u'1' or preview_obj[i][j] == u'2' \
                                    ...
                                    if preview_obj[i][j] == u'0' or preview_obj[i][j] == u'':
                                        ...
                                        if user_id is not False:
                                            user_obj = self.env['res.users'].search([('id', '=', user_id)])
                                            if user_obj.id is not False:
                                                group_e = self.env.ref('hr_timesheet.group_hr_timesheet_user',
                                                                       False)  # Officer
                                                group_e.write({'users': [(3, user_obj.id)]})
                                    ...
                                    if user_id is not False:
                                        ...
                                        if user_obj.id is not False:
                                            user_obj.unlink()
                                            lst_err.append(
                                                "The value of Timesheets column must be '0,1,2,3 or null' in row %s." % (
                                                    i + 1))
                                ...
                                if preview_obj[i][j] == u'0' or preview_obj[i][j] == u'1' or preview_obj[i][j] == u'2' \
                                    ...
                                    if preview_obj[i][j] == u'0':
                                        ...
                                        if user_id is not False:
                                            ...
                                            if user_obj.id is not False:
                                                ...
                                                group_e.write({'users': [(3, user_obj.id)]})
                                                group_env = self.env.ref('base.group_erp_manager',
                                                                         False)  # Access Rights
                                                group_env.write({'users': [(3, user_obj.id)]})
                                else:
                                    user_id = import_result['ids'][i]
                                    ...
                                    if user_id:
                                        ...
                                        if user_obj.id is not False:
                                            user_obj.unlink()
                                            lst_err.append(
                                                "The value of Administration column must be"
                                                " '0,1,2,3 or null' in row %s." % (
                                                    i + 1))

                        elif headers_obj[j].lower() == u'home_action' or headers_obj[j].lower() == u'home action':
                            for i in range(0, length_pre):
                                if (preview_obj[i][j]).lower() == u'project' or (
                                        preview_obj[i][j]).lower() == u'projects':
                                    user_id = import_result['ids'][i]
                                    if user_id is not False:
                        ...
                        elif headers_obj[j].lower() == u'contact creation' or headers_obj[
                            j].lower() == u'contact_creation':
                            for i in range(0, length_pre):
                                if preview_obj[i][j] == u'0' or preview_obj[i][j] == u'1' or preview_obj[i][j] == u'2' \
                                        or preview_obj[i][j] == u'3' or preview_obj[i][j] == u'':
                                    if preview_obj[i][j] == u'0' or preview_obj[i][j] == u'':
                                        ...
                                        if user_id is not False:
                                            user_obj = self.env['res.users'].search([('id', '=', user_id)])
                                            if user_obj.id is not False:
                                                group_e = self.env.ref('base.group_partner_manager',
                                                                       False)  # Contact Creation
                                                group_e.write({'users': [(3, user_obj.id)]})
                                    ...
                                    if user_id is not False:
                                        ...
                                        if user_obj.id is not False:
                                            user_obj.unlink()
                                            lst_err.append(
                                                "The value of Contact Creation column must be "
                                                "'0,1,2,3 or null' in row %s." % (
                                                    i + 1))
...
#
#     @api.model
#     def export(self, lst_err):
#         with contextlib.closing(cStringIO.StringIO()) as buf:
#             writer = csv.writer(buf, delimiter=";")
#             for i in range(0, len(lst_err)):
#                 writer.writerow((lst_err[i]))
...
#     @api.model
#     def create(self, vals):
#         res = super(Test, self).create(vals)
#         return res
