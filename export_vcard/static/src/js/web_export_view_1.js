odoo.define('web_export_view_vcard', function (require) {
"use strict";
    ...
    Sidebar.include({
        ...
        redraw: function () {
            ...
            if (self.getParent().ViewManager.active_view.type == 'list') {
                self.$el.find('.o_dropdown').last().append(QWeb.render('viewsss', {widget: self}));
                self.$el.find('.export_vcard').on('click', self.on_sidebar_export_treeview_vcard);
            }
        },

        on_sidebar_export_treeview_vcard: function () {
