    ...
    def from_data_employee(self, fields, rows):
        ...
        for data in rows:
            ...
            for lst_data in lst_data2:
                # lst_data = base64.encodestring(lst_data)
                writer.writerow(("BEGIN", "VCARD"))
                writer.writerow(("VERSION", "3.0"))
                writer.writerow(("N", lst_data[0].encode('utf-8') if lst_data[0] else ''))
                writer.writerow(("FN", lst_data[0].encode('utf-8') if lst_data[0] else ''))
                writer.writerow(("TEL;TYPE=CELL", lst_data[1] if  lst_data[1] else ''))
                writer.writerow(("TEL;TYPE=WORK", lst_data[2] if lst_data[2] else ''))
                writer.writerow(("EMAIL;TYPE=WORK", lst_data[3] if lst_data[3] else ''))
                writer.writerow(
                    ("ORG;CHARSET=UTF-8", lst_data[5] if lst_data[5] else ''))
                writer.writerow(("TITLE", lst_data[6].encode('utf-8') if lst_data[6] else ''))
    ...
    def from_data_contact(self, fields, rows):
        ...
        for data in rows:
            ...
            for lst_data in lst_data2:
                ...
                writer.writerow(("EMAIL;TYPE=WORK", lst_data[3] if lst_data[3] else ''))
                writer.writerow(
                    ("ORG;CHARSET=UTF-8", lst_data[4] if lst_data[4] else ''))
                writer.writerow(("TITLE", lst_data[5].encode('utf-8') if lst_data[5] else ''))
                writer.writerow(("END", "VCARD"))
